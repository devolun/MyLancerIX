...
public class AddPartActivity extends Activity {
	...
	public void onClick(View view) {
		...
		int valServiceChange = Integer.parseInt(serviceChange.getText()
				.toString());
		...
		if (nameOfPart.length() == 0) {
			Toast.makeText(getApplicationContext(), string.hint_name_of_part,
					Toast.LENGTH_SHORT).show();

		}
		if (oldChange.getText().length() == 0) {
			Toast.makeText(getApplicationContext(), string.hint_old_change,
					Toast.LENGTH_SHORT).show();

		}
		if (serviceChange.getText().length() == 0) {
			Toast.makeText(getApplicationContext(), string.hint_service_change,
					Toast.LENGTH_SHORT).show();
		...
		if (textCatalogeNameOfPart.getText().length() == 0) {
			...
			newValues.put(DatabaseHelper.PART_CATALOG_COLUMN,
					catalogeNameOfPart);
			newValues.put(DatabaseHelper.PART_SERVICE_CHANGE_COLUMN,
					valServiceChange);
			newValues.put(DatabaseHelper.PART_OLD_CHANGE_COLUMN, valOldChange);
			newValues.put(DatabaseHelper.PART_NEXT_CHANGE_COLUMN, valOldChange
					+ valServiceChange);
